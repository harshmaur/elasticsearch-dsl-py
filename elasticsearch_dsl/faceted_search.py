...
from .aggs import Terms, DateHistogram, Histogram, Tscript
...
AGG_TO_FILTER = {
	...
	Tscript: lambda a, v: F('script', **{a.script: v}),
...
BUCKET_TO_DATA = {
	...
	Tscript: lambda bucket, filter: (bucket['key'], bucket['doc_count'], bucket['key'] in filter),
...
class FacetedSearch(object):
	...
	def add_filter(self, name, value):
		...
		if not value:
			return
		# if "Tscript" in str(self.facets[name]):
